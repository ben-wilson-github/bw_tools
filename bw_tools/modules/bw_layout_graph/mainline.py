...
def run_mainline(branching_nodes: List[LayoutNode], branching_output_nodes: List[LayoutNode]):
    ...
    for _ in range(ITERATIONS):
        for branching_node in branching_nodes:
            ...
            if branching_node.identifier == 1420227963:
                print('as')
            
            # Try to get an easy mainline node. Basically a branching output
            potential_mainline_nodes: List[LayoutNode] = list()
            potential_mainline_nodes.append(branching_node.input_nodes[0])
            for input_node in branching_node.input_nodes:
                if input_node.pos.x < potential_mainline_nodes[0].pos.x:
                    potential_mainline_nodes = [input_node]
                elif input_node.pos.x == potential_mainline_nodes[0].pos.x:
                    potential_mainline_nodes.append(input_node)
            
            if len(potential_mainline_nodes) == 1:
                mainline_node = potential_mainline_nodes[0]
            else:
                found = False
                for n in potential_mainline_nodes:
                    if n.has_branching_outputs:
                        mainline_node = n
                        found = True
                        break
                if not found:
                    cds: List[bw_chain_dimension.ChainDimension] = get_chain_dimensions_full_chain(branching_node)
                    cds.sort(key=lambda x: x.bounds.left)
                    mainline_node: LayoutNode = get_mainline_node_from_left_most_bound(cds)
