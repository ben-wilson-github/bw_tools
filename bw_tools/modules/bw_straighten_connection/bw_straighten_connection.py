...
def get_from_settings_file(file_path: str, setting: str) -> Any:
    try:
        ...
        raise KeyError(
            f"Unable to get {setting} from settings file. It was not found inside the file."
        )

    except FileNotFoundError:
        raise FileNotFoundError(
            f"Unable to open {file_path}. The file was not found"
        )
    else:
        return ret
...
def straighten_connection(
    ...
    threshold = get_from_settings_file(
        __file__.replace(".py", "_settings.json"), "Distance From Node"
    )
